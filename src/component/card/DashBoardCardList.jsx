...
function DashBoardCardList(props) {
    ...
    return (
        <Card className={`list-card rcnt_act_card ${selectedCard === props.id ? 'selected' : ''}`} style={{ padding: !props?.colorWhite ? "" : 0 }}>
            <CardContent className="dash-c-body" style={{ backgroundColor: !props?.colorWhite ? "" : props?.colorWhite && selectedCard === props.id ? "" : "white" }} onClick={() => handleClick(props.type, props.id, props.content_type)}>
                <Link to={props.hasOwnProperty("contentId") ? `/Feeddetail/content/${props?.contentId}` : props.hasOwnProperty("contentDetail") ? `/content-details/${props?.contentDetail}` : props?.underScoreid && `/sourced-content-detail/${props?.underScoreid}`}>
                    <div className="list-in d-flex align-items-start gap-2">
                        <div className="rateReview_content">
                            {props.reviewType && <span className="rateView-type">
                                <span className="volCount">1</span>
                                <img className="" src={props.reviewType} alt="review type" />
                                </span>}
                            {props.imgtype === "audio" ? (
                                <div className="cstm_icn_wrpr">
                                    <img className="list-card-img me-0" src={props.imgl} alt="audio icon" />
                                </div>
                            ) : (
                                <img className="list-card-img" src={props.imgl} alt="content image" />
                            )}
                        </div>
                        <div className="list-in-txt pt-1">
                            ...
                            <Typography sx={{ fontSize: 12 }} color="#9DA3A3" gutterBottom className="crd_time d-flex align-items-center mb-0 txt_mdm">
