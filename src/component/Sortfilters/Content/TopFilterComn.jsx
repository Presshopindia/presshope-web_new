...
const TopFilterComn = ({
  ...
  const handleClickValues = (field, values, type) => {
    const existingIndex = multiFilter.findIndex(
      ...
      <div className="filter_wrap">
        <div className="srt_fltr_hdr">
          ...
          <div className="notf_icn_wrp">
            <a className="link" onClick={() => handleClose(false)}>
              Clear all
            </a>
          </div>
        </div>
        ...
        <div className="sort_list">
          ...
          ${multiFilter?.some((item) => item.values === true) ? "active" : ""}`}
            ...
            onClick={() => handleClickValues("content_under_offer", true, 2)}
          >
          ...
          <div
            className={`sort_item ${
              multiFilter?.some((item) => item.values === "exclusive")
                ? "active"
                : ""
            }`}
            style={{ cursor: "pointer" }}
            onClick={() => handleClickValues("type", "exclusive", 4)}
          >
          ...
          <div
            className={`sort_item ${
              multiFilter?.some((item) => item.values === "shared")
                ? "active"
                : ""
            }`}
            style={{ cursor: "pointer" }}
            onClick={() => handleClickValues("type", "shared", 5)}
          >
          ...
          {/* <div className="d-flex flex-column gap-2">
            {categoryId &&
              categoryId.map((curr, index) => {
                return (
                  <div
                    className={`sort_item ${
                      multiFilter?.some(
                        (item) => item.values === categoryId[index]?._id
                      )
                        ? "active"
                        : ""
                    }`}
